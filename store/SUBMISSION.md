...
This produces `dist/scrollrig-1.1.0.zip` containing only what the extension needs — `manifest.json`, the three scripts, the panel, and the icons. The README, store docs and design files are deliberately excluded; shipping them would only enlarge the package and give reviewers more surface to question.

To check what's in the zip before uploading:

```bash
unzip -l dist/scrollrig-1.1.0.zip
```
...
- [ ] **Upload** `dist/scrollrig-1.1.0.zip`
